...
#define CMD_CALL_ENTER     5  // Renumbered
#define CMD_CALL_RETURN    6  // Renumbered
#define CMD_FINALIZE       7  // Renumbered
#define CMD_GET_AUTH       8  // Renumbered
...
void cflat_finalize_and_print(void) {
    ...
    printf("Final Hash: ");
    for (int i = 0; i < 32 && i < auth_size; i++) {
        printf("%02x", auth[i]);
    }
    printf("\n");
    ...
    if (auth_size > 36) {
        ...
        size_t offset = 36;
        for (uint32_t i = 0; i < loop_count && offset + 76 <= auth_size; i++) {
            uint64_t loop_id = *(uint64_t*)&auth[offset];
            uint32_t iterations = *(uint32_t*)&auth[offset + 72];
            
            printf("  Loop 0x%lx: %u iterations\n", loop_id, iterations);
            offset += 76; // 8 + 32 + 32 + 4
        }
    }
